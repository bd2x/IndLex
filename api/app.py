...
APP_ENV = os.environ.get("APP_ENV", "production")  # "local" for local http testing
...
def _migrate_users_is_active_to_boolean(cur):
    cur.execute(
        """
        SELECT data_type
    ...
    # Convert legacy int (0/1) to boolean
    cur.execute("ALTER TABLE users ALTER COLUMN is_active DROP DEFAULT")
    cur.execute(
        """
        ALTER TABLE users
...
def init_db():
    ...
    try:
        ...
        _migrate_users_is_active_to_boolean(cur)

        # Seed roles (viewer is harmless even if unused)
        cur.execute(
...
def login():
    ...
    try:
        ...
        # frontend expects userid (not user_id)
        return jsonify(
            {"status": "logged_in", "userid": user.id, "username": user.username, "roles": roles}
        ), 200
    finally:
        if conn:
            put_conn(conn)
...
def me():
    ...
    return jsonify(
        {"authenticated": True, "userid": current_user.id, "username": current_user.username, "roles": roles}
    ), 200
...
def admin_create_user():
    ...
    is_active = parse_bool(payload.get("isActive", True), default=True)
    ...
    try:
        ...
        conn.commit()
        return jsonify({"status": "user_created", "userid": user_id, "username": username}), 201
    finally:
        if conn:
            put_conn(conn)
...
def admin_set_active(user_id: int):
    ...
    is_active_bool = parse_bool(payload.get("isActive"), default=True)
    ...
    try:
        ...
        conn.commit()
        return jsonify({"status": "active_updated", "userid": user_id, "isActive": is_active_bool}), 200
    finally:
        if conn:
            put_conn(conn)
...
def admin_set_roles(user_id: int):
    ...
    try:
        ...
        conn.commit()
        return jsonify({"status": "roles_updated", "userid": user_id, "roles": list(new_roles)}), 200
    finally:
        if conn:
            put_conn(conn)
        ...
        if language:
            cur.execute(
                ...
                SELECT
                  ...
                  root,
                  meaning,
                  best_headword AS bestheadword,
                  similarity_score AS similarityscore,
                  distance_to_root AS distancetoroot,
    ...
    try:
        ...
        cur.execute(
            ...
            SELECT
              ...
              l.language,
              l.meaning,
              l.best_headword AS bestheadword,
              l.distance_to_root AS distancetoroot,
              l.similarity_score AS similarityscore,
              l.created_at AS createdat,
              l.created_by_user_id AS createdbyuserid,
              u.username AS insertedby
            FROM lookups l
    ...
    try:
        ...
        cur.execute(
            ...
            SELECT
              ...
              COUNT(DISTINCT root) AS distinctroots,
              SUM(COALESCE(similarity_score, 0)) AS totalsimilarityscore,
              AVG(similarity_score) AS avgsimilarityscore
            FROM lookups
            """
        )
        row = cur.fetchone() or {}
        ...
        cur.execute(
            ...
            SELECT
              ...
              END AS percentsanskrit
            FROM lookups
            GROUP BY COALESCE(language, 'Unknown')
            ORDER BY totalroots DESC
            """
        )
        by_lang = cur.fetchall()

        out = dict(row)
        out["bylanguage"] = by_lang
